...
- **Remaining differences from the 28 unified tools**: failures surface as `StructuredError` payloads rather than raising through `classify_tool_error`; `providers/http_client.py` is still synchronous (offloaded per-call, not natively async). Known data-source limits stand: MKK/VAP parsing is regex-label-based against page text, and Takasbank KYP filter params (`institutional_investor_type`/`fund_type`/`date`) are echoed onto records but the static page is usually JS-gated, so KYP calls typically return `takasbank_kyp_parse_unavailable`.
