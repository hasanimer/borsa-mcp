...
- **Runtime wrapper**: each tool routes through `tools/_runtime.run_tool`, which offloads the blocking synchronous `httpx`/SQLite work to a worker thread (`asyncio.to_thread`, so the FastMCP event loop is never blocked) and applies `response_shaper.strip_nulls` for parity with the 28 unified tools. Services are module-level singletons (constructed once at import).
- **Remaining differences from the 28 unified tools**: failures surface as `StructuredError` payloads rather than raising through `classify_tool_error`; `providers/http_client.py` is still synchronous (offloaded per-call, not natively async).
- **Scraper behavior**: Takasbank KYP `records()` scopes rows to the table whose nearby heading/caption matches the requested `data_type` (so `get_institutional_portfolio_distribution` and `get_fund_type_portfolio_sizes` return distinct rows on server-rendered pages), falling back to all tables when no heading matches, and drops header/label-only rows (those with no parseable numeric cell). MKK/VAP `market_summary()` validates ownership ratios to the [0,100] range, discarding regex mismatches. Residual data-source limits (cannot be verified without live access — both sites WAF-block automated requests): the KYP `institutional_investor_type`/`fund_type`/`date` params are echoed onto records but not used for row-level column filtering, the live KYP page is often JS-gated (→ `takasbank_kyp_parse_unavailable`), and MKK/VAP figures come from label-based regex over page text.
