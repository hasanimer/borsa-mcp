...
def register(app):
    ...
    async def get_equity_investor_summary(): return await run_tool(_service.equity_summary)
    ...
    async def get_domestic_foreign_ownership(): return await run_tool(_service.domestic_foreign_ownership)
    ...
    async def get_top_stocks_by_investor_count(limit: Annotated[int, Field(default=10, ge=1, le=100)] = 10): return await run_tool(_service.top_stocks_by_investor_count, limit)
    ...
    async def get_stock_investor_trend(symbol: Annotated[str, Field(min_length=2, examples=["THYAO"])]): return await run_tool(_service.stock_investor_trend, symbol)
