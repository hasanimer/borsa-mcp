...
class InstitutionalPortfolioService:
    ...
    def distribution(self, institutional_investor_type, fund_type="Tümü", date=None): return self.provider.records(KypDataType.PORTFOLIO_DISTRIBUTION.value, institutional_investor_type, fund_type, date)
    def fund_type_sizes(self, institutional_investor_type="Yatırım Fonları", date=None): return self.provider.records(KypDataType.FUND_TYPE_SIZES.value, institutional_investor_type, "Tümü", date)
    ...
    def equity_exposure(self, institutional_investor_type="Yatırım Fonları", fund_type="Tümü", date=None):
        ...
        if "records" in data:
            ...
            if not data["records"]:
                ...
                meta.setdefault("warnings", []).append("No equity exposure row was detected in parsed KYP records.")
        return data
